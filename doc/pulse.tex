...
%
%
%The pulse of a breaking story on social media can be modeled as a double exponential pulse,
%
%\begin{equation}
%r(t) = r_0(1-\exp(-\alpha t))\exp(-\beta t)
%\label{eq:pulse}
%\end{equation}
%
%In this Eq \ref{eq:pulse}, $r_0$ is a rate magnitude parameter, $\alpha$ is associated with growth of the story and $\beta$ characterizes the exponential decay rate of the story. These are the Social Media Pulse fit parameters.
%
%\section{Model Details}
%\label{sec:parameters}
%
%This model is motivated by both the desire to compare different stories on a given firehose and also to compare stories across firehoses with analytical consistency.  The double-exponential model has two attractive features:
%
%\begin{itemize}
%\item The Social Media Pulse fit parameters can be quickly and reliably computed with common least-squares numerical methods
%\item Time-to-peak, story half-life, total story activity volume, and average time of response are straight forward to compute given the fit parameters
%\end{itemize}
%
%A reasonable strategy for fitting Eq. \eqref{eq:pulse} is to bucket activity data to estimate the activity rate on regular time intervals.  The rate is give by the number of activities per bucket divided by the bucket width, $\delta t_{bucket}$.  The quality of the estimates depends on the bucket size as related to the timescales of the pulse. That is, we can determine the activity rate as long as the bucket width is small compared to the timescale,
%
%\begin{equation} 
%\delta t_{bucket} << t_{peak}
%\label{eq:limit1}
%\end{equation}
%
%Practically, it is often adequate to have $t_{peak}$ a few times $t_{bucket}$.  Another limit is set by the requirement that there not be too few activities per period. If the number of activities is too low, the estimate of rate becomes very uncertain (estimates of averages vary as $\frac{1}{\sqrt{\text{sample size}}}$). So we have, 
%
%\begin{equation} 
%\text {rate} >> 1 \text { activity per period}
%\label{eq:limit2}
%\end{equation}
%
%In practice, we choose $\delta t_{bucket}$ to satisfy both Eq \ref{eq:limit1} and \ref{eq:limit2}.  If activities are too sparse, i.e. $\delta t{bucket} \approx t_{peak}$ contains only a few events, another strategy may be required to make an estimate of the Social Media Pulse.
%
%We can use common curve fitting techniques to compute the parameters of Eq \ref{eq:pulse}. For example, in Python, use the Scipy package,  optimize module method \texttt{optimize.leastsq(...)} to quickly perform the fit. To make sure we fit the pulse quickly and accurately, it is important to window the data around the pulse.
%
%It is also convenient to provide an initial guess for the fit calculate from the sample data.  In the following, the left-hand values are initial values for the fit and the right-hand values are calculated from the pulse sample data.
%
%\begin{align}\label{eq:guess}
%r_0 & = r_{max}\\
%\alpha & = \frac{10}{t_{range}}\\
%\beta & = \frac{1}{t_{range}}\\
%t_0 & = t(r_{max})
%\end{align}
%where $t_{range} = t_{max} - t_{min}$, the data window size, and $t_0$ is required if you are fitting the pulse in the form of Eq \ref{eq:pulseOffset}.
%
%\begin{figure}[ht]
%\includegraphics[width=2.8 in]{twitterPulse_fits_limit.png}
%\caption{Twitter: Mexico Earthquake Social Media Pulse with fitted Social Media Pulse.}
%\label{fig:TwitterLimitedFit}
%\end{figure}
%
%Once we have fit the function, use $r_0$, $\alpha$ and $\beta$ to calculate the relevant parameters that characterize Social Media Pulse.
%
%\textbf{Time-to-Peak.}  The peak time is the time when the activity rate reaches its peak.  This is derived by maximizing Eq \ref{eq:pulse} . The result is,
%
%\begin{equation}
%t_{time-to-peak} = \frac{1}{\alpha} \ln(\frac{\alpha}{\beta} + 1)
%\label{eq:peak}
%\end{equation}
%
%The lower the time-to-peak, the faster the story reached a majority of the interested parties on the social media platform. This is a useful proxy for comparing the speeds at which various stories get out.
%
%\textbf{Story Activity Volume.}  The story activity volume is the integral of the rate for the relevant time,
%
%\begin{equation}
%S_{vol} = \int_0^\infty r(t) dt = \frac{r_0 \alpha}{\alpha \beta + \beta^2}
%\label{eq:mass}
%\end{equation}
%This is a useful proxy for comparing story size.
%
%\textbf{Story Half-life.}  Attention wanes after the story peaks. The half-life is the point where the activity rate has declined to half the peak rate. Therefore, we define $t_{1/2}$ such that $r(t_{1/2}) = \frac{1}{2} r(t_{time-to-peak})$.  This relationship results in a transcendental equation that is rapidly solved by starting with $t_0 = 2(t_{time-to-peak}$ and iterating, 
%
%\begin{equation}
%\begin{split}
%t_{half, i+1} & = \\
%&\frac {r_0 (1-\exp(-\alpha t_i))  \exp(-\beta t_i) - 0.5*r_{peak}}{(\alpha + \beta) \exp(-(\alpha + \beta) t_i) - \beta \exp(-\beta t_i)}
%\end{split}
%\label{eq:iter}
%\end{equation}
%until your reach the desired accuracy (usually $\le 10$ iterations). Half-life is a useful proxy for story longevity. 
%
%\textbf{Average Response Time.}  Average response time is the balance point between the volume in the tall peak and activity long after the event, in the skinny tail. The average response time is given by,
%
% \begin{equation}
%t_{avg} = \frac {\alpha + 2\beta}{\beta(\alpha+\beta)}
%\label{eq:avg}
%\end{equation}
%until your reach the desired accuracy (usually $\le 10$ iterations).
%
%It is sometimes convenient to allow the fitting computation to give the starting time of the pulse in addition to the other fit parameters. This can be accomplished by adding the parameters $t_0$, the start time, to Eq \ref{eq:pulse} like this,
%
%\begin{equation}
%r(t) = r_0(1-\exp(-\alpha (t - t_0))\exp(-\beta (t-t_0))
%\label{eq:pulseOffset}
%\end{equation}
%where all of the time parameters above are now calculated relative to $t_0$.
